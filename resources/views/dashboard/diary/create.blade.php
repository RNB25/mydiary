          ...
          <div class="flex-none w-full max-w-full px-3">
            <div class="relative flex flex-col min-w-0 break-words bg-white border-0 border-transparent shadow-soft-xl rounded-2xl bg-clip-border">
              ...
              <div class="flex-auto px-6 pt-4 pb-6">
                <form method="POST" action="{{ route('catatan.store') }}" class="space-y-6">
                  @csrf

                  {{-- Mood --}}
                  <div>
                      <label for="mood" class="block text-sm font-medium text-slate-600 mb-2">
                          Mood
                      </label>
                      <select name="mood" id="mood" required
                          class="block w-full rounded-lg border border-gray-300 bg-white py-2 px-3 text-sm shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                          ...
                          {{-- @foreach ($moods as $mood)
                              <option value="{{ $mood->name }}">
                                  {{ $mood->emoticon }} {{ ucfirst($mood->name) }}
                              </option>
                          @endforeach --}}
                      </select>
                  </div>
                  ...
                  {{-- Title --}}
                  <div>
                      <label for="title" class="block text-sm font-medium text-slate-600 mb-2">
                          Hari ini tentang apa?
                      </label>
                      <input type="text" name="title" id="title" required
                          class="block w-full rounded-lg border border-gray-300 py-2 px-3 text-sm shadow-sm placeholder-gray-400 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                          placeholder="Contoh: Jalan-jalan ke pantai" />
                  </div>

                  {{-- Content --}}
                  <div>
                      <label for="content" class="block text-sm font-medium text-slate-600 mb-2">
                          Gimana cerita kamu?
                      </label>
                      <textarea name="content" id="content" rows="5" required
                          class="block w-full rounded-lg border border-gray-300 py-2 px-3 text-sm shadow-sm placeholder-gray-400 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                          placeholder="Ceritakan harimu di sini..."></textarea>
                  </div>
